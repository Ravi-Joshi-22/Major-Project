  ...
  JobOpening.find({
    ...
    .exec(function(err, fetchedData) {
      if (err) {
        callback({
          type: CONSTANTS.ERROR_TYPES.DB_ERROR,
          msg: 'Failed to get eligible openings records',
          errorDetail: JSON.stringify(err),
        });
      } else {
        callback(null, fetchedData);
    ...
    [
      ...
      function(fetchedInterviewee, quaskills, tracks, waterfallCallback) {
        userHelper.getTotalExperience(userId, function(err, experience) {
          const exp = experience[0].total_experience;
 ...
 * @param {Function} callback having error and fetched data
 */
function extractUpcomingApplied(tracks, callback) {
  const ed = new Date();
  JobOpening.find({
    end_date: {
      $gt: ed,
    ...
    .exec(function(err, fetchedData) {
      if (err) {
        callback({
          type: CONSTANTS.ERROR_TYPES.DB_ERROR,
          msg: 'Failed to get upcoming applied openings records',
          errorDetail: JSON.stringify(err),
        });
      } else {
        callback(null, fetchedData);
...
/**
 * Function to first getting all interviewee tracks, further fetching applied openings
 * @param {ObjectId} userId
 * @param {Function} callback having error and data
 */
function upcomingAppliedOpenings(userId, callback) {
  async.waterfall(
    [
      function(waterfallCallback) {
        getAllIntervieweeTracks(userId, function(err, tracks) {
      ...
      function(tracks, waterfallCallback) {
        extractUpcomingApplied(tracks, function(err, Opening) {
          waterfallCallback(err, Opening);
        });
      },
    ],
 ...
 * @param {Function} callback having error and fetched data
 */
function extractCurrentApplied(tracks, callback) {
  const ed = new Date();
  JobOpening.find({
    end_date: {
      $gte: ed,
    ...
    .exec(function(err, fetchedData) {
      if (err) {
        callback({
          type: CONSTANTS.ERROR_TYPES.DB_ERROR,
          msg: 'Failed to get current date applied openings records',
          errorDetail: JSON.stringify(err),
        });
      } else {
        callback(null, fetchedData);
...
/**
 * Function to first getting all interviewee tracks, further fetching current applied openings
 * @param {ObjectId} userId
 * @param {Function} callback having error and data
 */
function currentAppliedOpenings(userId, callback) {
  async.waterfall(
    [
      function(waterfallCallback) {
        getAllIntervieweeTracks(userId, function(err, tracks) {
      ...
      function(tracks, waterfallCallback) {
        extractCurrentApplied(tracks, function(err, Opening) {
          waterfallCallback(err, Opening);
        });
      },
    ],
...
module.exports = {
  ...
  upcomingAppliedOpenings: upcomingAppliedOpenings,
  currentAppliedOpenings: currentAppliedOpenings,
};
