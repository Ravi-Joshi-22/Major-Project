...
function extractUpcomingApplied(userId, tracks, callback) {
  ...
  JobOpening.find({
    ...
    .populate({
      ...
      select: '_id user_id interview_status',
    })
    .sort({ end_date: 1 })
    .exec(function(err, fetchedData) {
      if (err) {
...
/**
 * Get Results of all the opening where user has
 * @param {Array} tracks applied tracks
 * @param {objectId} userId user object _id
 * @param {function} callback error and opening with result details
 */
function getResult(tracks, userId, callback) {
  let ed = new Date();
  JobOpening.find(
    {
      end_date: {
    ...
    { company_id: 1, interviewees: 1, position: 1, start_date: 1, end_date: 1 }
  )
    .populate({
      path: 'company_id',
      select: 'company_name',
    ...
    .sort({ end_date: -1 })
    .exec(function(err, fetchedOpenings) {
      if (err) {
        callback({
          type: CONSTANTS.ERROR_TYPES.DB_ERROR,
          msg: 'Failed to fetch result Details',
          errorDetail: JSON.stringify(err),
        });
      } else {
        callback(null, fetchedOpenings);
      }
    });
}

function getAllResults(userId, callback) {
  async.waterfall(
    [
      function(waterfallCallback) {
        getAllIntervieweeTracks(userId, function(err, tracks) {
      ...
      function(tracks, waterfallCallback) {
        getResult(tracks, userId, function(err, results) {
