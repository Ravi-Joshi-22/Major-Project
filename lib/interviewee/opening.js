...
/**
 * Function to get result of logged interviewee for an opening which is requested and is cloed by company
 * @param {ObjectId} openingId _id of Opening
 * @param {*} userId _id of User
 * @param {*} callback Function having two parameters - err and fetched Interviewee
 */
function getResult(openingId, userId, callback) {
  let ed = new Date();
  JobOpening.findById(openingId)
    .populate({
      path: 'interviewees',
    ...
    .exec(function(err, fetchedOpening) {
      if (err) {
        ...
        callback(null, fetchedOpening.interviewees[0]);
      }
    });
}
