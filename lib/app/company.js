  ...
  newCompany.save(function(companySaveError, savedCompany) {
    if (companySaveError) {
      callback({
        ...
        msg: 'Unable to create company account , please try again !',
  ...
  Company.findById(companyId, function(err, doc) {
    if (err) {
      callback({
        type: CONSTANTS.ERROR_TYPES.DB_ERROR,
        msg:
          'No such company exists with the given Id , please insert correct Id',
    ...
    doc.save(function(errInSave, savedUser) {
      if (errInSave) {
        callback({
          type: CONSTANTS.ERROR_TYPES.DB_ERROR,
          msg: 'Failed to update a new user in company, please try again !',
