    ...
    function(err, generatedInstance) {
      if (err) {
        ...
        callback({
          ...
          msg: 'First Create some openings for these company',
          errorDetail: 'No openings exists for these company',
        });
      } else {
...
/**
 * Function to calculate the number of openings created in running and last month by company
 * @param {ObjectId} companyId Company's objectId
 * @param {Function} callback Function having two parameter - error and object holding number of openings created in the running amd last month by company
 */
function numberOfRunningAndLastMonthCreatedOpenings(companyId, callback) {
  let ed = new Date();
  ...
  JobOpening.aggregate(
    [
      ...
      {
        $match: {
          '_id.month': { $in: [last_month, running_month] },
          '_id.year': { $in: [last_year, running_year] },
        },
      },
      {
        $project: {
        ...
        $project: {
          no_of_openings_created_in_running_month: {
            $cond: [
              ...
              '$running_month_openings.numberofcandidates',
              0,
            ],
          },
          no_of_openings_created_in_last_month: {
            $cond: [
              ...
              '$last_month_openings.numberofcandidates',
              0,
            ],
          },
        },
    ...
    function(err, generatedInstance) {
      if (err) {
        callback({
          type: CONSTANTS.ERROR_TYPES.DB_ERROR,
          msg:
            'Failed to group number of openings based on the last and running months created openings',
          errorDetail: JSON.stringify(err),
        });
      } else {
        callback(null, generatedInstance[0]);
    ...
    [
      ...
      function(populatedData, waterfallCallback) {
        numberOfCandidatesGivingInterview(populatedData._id, function(
          ...
          let obj = {
            no_of_candidates_giving_interview_per_opening: generatedData,
          };
      ...
      function(populatedData, waterfallCallback) {
        numberOfRunningAndLastMonthCreatedOpenings(populatedData._id, function(
          err,
          generatedData
        ) {
          let newData = Object.assign({}, populatedData, generatedData);
